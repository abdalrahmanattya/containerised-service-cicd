...
Project 5 runs on Docker Desktop Kubernetes. The local control-plane node is
...
`linux/amd64` and `linux/arm64` manifests for each new release. Project 5 will
continue to reference the resulting top-level immutable digest.
...
- Project 5 must update its digest after each multi-architecture release.
